...
//   if (status === "verified" || approvalStatus === "approved" || user.isVerified) return "verified";
//   return "awaiting";
// }
...
//     table.innerHTML = organizers.map((user) => `
//       <tr>
//         <td>${user.fullName || user.name || user.organizationName || "Unnamed Organizer"}</td>
//         <td>${user.email || "—"}</td>
//         <td>${getLocationText(user)}</td>
//         <td><span class="status-badge status-${statusOf(user)}">${statusOf(user)}</span></td>
//         <td><a href="verification-details.html?id=${encodeURIComponent(user._id || user.id)}">View Details</a></td>
//       </tr>
//     `).join("") || `<tr><td colspan="5">No organizations found.</td></tr>`;
//   } catch {
//     table.innerHTML = `<tr><td colspan="5">Failed to load verification queue.</td></tr>`;
//   }
// });
